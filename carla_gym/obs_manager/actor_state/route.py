...
class ObsManager(ObsManagerBase):
    ...
    def _define_obs_space(self):
        self.obs_space = spaces.Dict({
            'lateral_dist': spaces.Box(low=0.0, high=2.0, shape=(1,), dtype=np.float64),
            'angle_diff': spaces.Box(low=-2.0, high=2.0, shape=(1,), dtype=np.float64),
            'route_locs': spaces.Box(low=-5.0, high=5.0, shape=(self._route_steps*2,), dtype=np.float64),
            'dist_remaining': spaces.Box(low=0.0, high=100, shape=(1,), dtype=np.float64)
        })
    ...
    def get_observation(self):
        ...
        np_d_vec = np.array([d_vec.x, d_vec.y], dtype=np.float64)
        wp_unit_forward = wp_transform.rotation.get_forward_vector()
        np_wp_unit_right = np.array([-wp_unit_forward.y, wp_unit_forward.x], dtype=np.float64)
