...
class ObsManager(ObsManagerBase):
    ...
    def _define_obs_space(self):
        ...
        self.obs_space = spaces.Dict({
            'acc_xy': spaces.Box(low=-1e3, high=1e3, shape=(2,), dtype=np.float64),
            'vel_xy': spaces.Box(low=-1e2, high=1e2, shape=(2,), dtype=np.float64),
            'vel_ang_z': spaces.Box(low=-1e3, high=1e3, shape=(1,), dtype=np.float64)
        })
