...
class RunRedLight:
    def __init__(self, carla_map, tl_handler, distance_light=30):
        self._map = carla_map
    ...
    def tick(self, vehicle, timestamp):
        ...
        ev_dir = ev_transform.get_forward_vector()
        ev_extent = vehicle.bounding_box.extent.x

        tail_close_pt = ev_transform.transform(carla.Location(x=-0.8 * ev_extent))
        tail_far_pt = ev_transform.transform(carla.Location(x=-ev_extent - 1.0))
        tail_wp = self._map.get_waypoint(tail_far_pt)

        info = None
        for idx_tl in range(self._tl_handler.num_tl):
            traffic_light = self._tl_handler.list_tl_actor[idx_tl]
            tl_tv_loc = self._tl_handler.list_tv_loc[idx_tl]
            ...
            for idx_wp in range(len(self._tl_handler.list_stopline_wps[idx_tl])):
                wp = self._tl_handler.list_stopline_wps[idx_tl][idx_wp]
                ...
                if tail_wp.road_id == wp.road_id and tail_wp.lane_id == wp.lane_id and dot_ve_wp > 0:
                    ...
                    stop_left_loc, stop_right_loc = self._tl_handler.list_stopline_vtx[idx_tl][idx_wp]
